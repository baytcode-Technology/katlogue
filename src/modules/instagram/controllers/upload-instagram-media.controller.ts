...
import {
  inferInstagramMediaKind,
  uploadInstagramChatMedia,
} from '../services/upload-instagram-media.service.js'

export const uploadInstagramMedia = asyncHandler(async (req: Request, res: Response) => {
  ...
  const kind = inferInstagramMediaKind({ kindRaw, mimeType, filename })

  if (kind === 'audio' && file.buffer.length < 256) {
    throw new AppError(400, 'Audio recording is too short or empty', 'INSTAGRAM_AUDIO_EMPTY')
  }

  const result = await uploadInstagramChatMedia({
    storeId,
