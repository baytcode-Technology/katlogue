...
import * as orderRepository from '../../modules/orders/repositories/order.repository.js'

import { toPublicStore } from '../../modules/storefront/types/public-store.types.js'

import { assertStorefrontAvailable, hasPremiumAccess } from '../lib/subscription.js'
...
export async function resolveStoreFromHost(
  ...
  try {
    ...
    if (!hasPremiumAccess(store)) {
      ...
      const monthlyOrderCount = await orderRepository.countOrdersInCurrentMonth(store.id)

      assertStorefrontAvailable(store, { monthlyOrderCount })
